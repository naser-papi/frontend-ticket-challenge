...
fs.copyFileSync("e2e-coverage/coverage-final.json", `${REPORTS_FOLDER}/from-cypress.json`);
fs.copyFileSync("unit-coverage/coverage-final.json", `${REPORTS_FOLDER}/from-jest.json`);
...
try {
  run([
    ...
    `nyc report --reporter lcov --reporter json-summary --report-dir ${FINAL_OUTPUT_FOLDER}`
  ]);
} catch (error) {
  console.log(error);
}
